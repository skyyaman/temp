...
Security
Insights
jd_scripts/jd_connoisseur.js /
@yuannian1112
yuannian1112 更新
Latest commit 39193bd 2 days ago
 History
 1 contributor
629 lines (618 sloc)  37.8 KB
...
!(async () => {
    ...
    let res = await getAuthorShareCode('xxxx')
    if (!res) {
        $.http.get({url: 'xxxx'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
        await $.wait(1000)
        res = await getAuthorShareCode('xxxx')
    }
